<script lang="ts">
	...
	import EditPasswordDialog from './EditPasswordDialog.svelte';
	...
	function savePassword(password: PasswordEntry) {
		if (password) {
...
{#if editingPassword}
	<EditPasswordDialog password={editingPassword} onSave={savePassword} onCancel={cancelEdit} />
{/if}

<div
	class="mb-4 rounded {isDarkTheme ? 'bg-gray-800' : 'bg-white'} px-4 pt-6 pb-8 shadow-md md:px-8"
>
					...
					<table class="min-w-full table-fixed divide-y divide-gray-200">
						...
						<tbody class="divide-y divide-gray-200 {isDarkTheme ? 'bg-white/10' : 'bg-gray-100 '}">
							{#each sitePasswords as password (password.id)}
								<tr>
									<td class="py-4 whitespace-nowrap md:px-6">
										{password.username}
									</td>
									<td class="py-4 whitespace-nowrap md:px-6">
									...
									<td class="py-4 text-sm font-medium whitespace-nowrap md:px-6">
										{#if selectedPassword?.id === password.id}
											<button
